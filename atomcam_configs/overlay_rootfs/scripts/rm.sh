...
if [ $FTPCLIENT = "on" ]; then
  if [ $FTPTRANSALARM = "on" ]; then
    if [ "$FILE" = "/tmp/alarm_record.mp4" ]; then
       LD_LIBRARY_PATH=/tmp/system/lib:/usr/lib /tmp/system/lib/ld.so.1 /tmp/system/bin/lftp -e "mkdir -p $FTPFOLDER/alarm_record; put -O $FTPFOLDER/alarm_record $FILE -o $FMT.mp4 ; quit" ftp://$FTPADDR -u $FTPUSER,$FTPPASS
    fi
  fi
fi
busybox rm $*
